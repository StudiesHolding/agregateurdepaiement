...
const restoreAndAddColumns = async () => {
  try {
    console.log("🔄 RESTAURATION ET AJOUT DE COLONNES");
    console.log("=" .repeat(60));
    console.log("🔄 Utilisation du backup existant");
    ...
    console.log(`📊 Backup contient ${backupData[0].count} enregistrements`);
    ...
    // Step 1: Restore data from backup to course_packages
    console.log("\n🔄 Étape 1: Restauration des données depuis le backup...");
    
    try {
      // Clear current course_packages
      ...
      `);
      console.log(`   ✅ ${backupData[0].count} enregistrements restaurés`);
  ...
  try {
    ...
    if (count > 0) {
      ...
      if (b2bActivePackages.length > 0) {
        ...
        console.log("✅ Vos données ont été restaurées et enrichies !");
        ...
        console.log("6. Vos 8 packages originaux sont préservés !");
...
// Run restore and add columns
restoreAndAddColumns();
